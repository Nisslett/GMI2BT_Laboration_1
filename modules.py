...
def guessing_number():
    ...
    while True:
        try_input = input(
            f"Guess a number betwenne {limit_lower} to {limit_upper} :")
        ...
        if try_input == rand_number:
            print(
                f"WOOHOO!!! Your guess of {try_input} was correct. Congratualtions!")
            break
        else:
            if rand_number > try_input:
                ...
                hint = "smaller"
            print(
                f"Your guess was incorrect , the nubmer your looking for is {hint}.")
